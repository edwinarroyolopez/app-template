...
import { CalendarClock, ChevronRight, Clock3, PackageCheck, UserRound } from 'lucide-react-native';
...
function deliveryLabel(process: ManufacturingProcess) {
  return process.productName || 'Linea sin producto';
}

function commitmentText(deliveryDate?: string) {
  if (!deliveryDate) return 'Compromiso sin fecha';
  return `Compromiso ${new Date(deliveryDate).toLocaleDateString('es-CO')}`;
}

export function ReadyForDeliveryOrderCard({ process, onPress, isTopPriority = false }: Props) {
  const delayed = deriveDelayedInfo(process);
  const isDelayed = Boolean(process.isDelayed ?? delayed.isDelayed);
  const delayedDays = Number(process.delayedDays ?? delayed.delayedDays ?? 0);

  const transporter = process.responsibleEmployee
  ...
  return (
    <Pressable style={[styles.card, isDelayed && styles.cardDelayed, isTopPriority && styles.cardTopPriority]} onPress={onPress}>
      <View style={styles.topRow}>
        <Text style={styles.productName} numberOfLines={1}>{deliveryLabel(process)}</Text>
      ...
      <View style={styles.transporterRow}>
        ...
        <Text style={styles.transporterText} numberOfLines={1}>{transporter}</Text>
      </View>
      ...
      <View style={styles.contextRow}>
        {isDelayed && delayedDays > 0 ? (
          ...
          <View style={styles.readyChip}>
            <PackageCheck size={12} color="#34D399" />
            <Text style={styles.readyChipText}>Listo para salida</Text>
          </View>
        )}

        <View style={styles.ctaWrap}>
          <Text style={styles.ctaText}>Entregar</Text>
